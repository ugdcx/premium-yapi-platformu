...
export function createApplication(data) {
  ...
  const application = {
    ...
    applicationNo: `BLAGG-${new Date().getFullYear()}-${String(applications.length + 1).padStart(3, "0")}`,
    status: "Yeni",
...
export function getApprovedUpdates() {
  return readItems(UPLOADS_KEY).filter(
    (upload) => upload.status === "approved" && upload.visible_to_customer && upload.customerDescription?.trim()
  );
}
